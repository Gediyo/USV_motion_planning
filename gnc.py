...
# D = Dmtx(Dlin,Dquad,nu) computes the 6x6 total Damping matrix
# given the linear and quadratic damping coefficients matrices and 
# the current body velocity vector
def Dmtx(Dlin,Dquad,nu):
    D = -1 * Dlin
    for i in range(6):
        D[i,i] += -1 * Dquad[i,i] * np.abs(nu[i])
    return D

def eulerang(eta):
    J = np.zeros(shape=(6,6), dtype=float)
    J[0:3, 0:3] = Rzyx( eta[3], eta[4], eta[5] )
    J[3:6, 3:6] = Tzyx(eta[3], eta[4])
    return J
